    ...
    return (
        <div ref={container} style={{ scrollSnapType: "x mandatory" }} className='relative  w-full h-full flex flex-row flex-1 gap-2 overflow-x-scroll md:overflow-hidden border-2  select-none '>
            <div className='hidden sticky z-30 top-1/2 left-2  md:flex items-center justify-center'>
                <button className='bg-gray-200 hover:bg-blue-400 px-4 py-2 rounded active:scale-90 duration-75  font-semibold' onClick={scrollLeft}> left{/*<FaArrowLeft />*/}</button>
            </div>
            ...
            <div className='hidden sticky z-30 top-1/2 right-2 bg-transparent  md:flex items-center justify-center'>   <button onClick={scrollRight} className='bg-gray-200 hover:bg-blue-400 px-4 py-2 rounded active:scale-90 duration-75 font-semibold' > right{/*<FaArrowRight />*/}</button></div>
        </div>
    );
}
