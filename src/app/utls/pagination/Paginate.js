  ...
  return (
    <div className="flex gap-2 mt-2 w-full">
      <button
        ...
        className="bg-base-300 border shadow rounded px-3 py-1 font-bold text-lg duration-100 active:scale-90"
      >
        {"<<"}
      </button>
      <button
      ...
      <form ref={ref} onSubmit={formPaginate}>
        <input
          ...
          type="number"
          name="page"
          className="bg-base-300 border border-black shadow rounded px-3 py-1 text-lg w-20"
          placeholder="page"
        ></input>
      </form>
      <button
...
const PageButton = ({ page, length }) => {
  return (
    <button className="bg-base-300 border shadow rounded px-3 py-1 ">
      {page}
    </button>
  );
};
